...
async def stance(ctx,x,y,turn,field,used,em):    
    if used not in typemoves.statusmove and x.ability=="Stance Change" and x.sword!=True:
        x.shield=False
        x.sword=True
        em.add_field(name=f"{x.icon} {x.nickname}'s Stance Change!",value="Aegislash changed to it's blade forme.")
    ...
    if used in typemoves.statusmove and x.ability=="Stance Change" and x.shield!=True:
        x.shield=True
        x.sword=False
        em.add_field(name=f"{x.icon} {x.nickname}'s Stance Change!",value="Aegislash changed to it's shield forme.")
...
async def attack(ctx,bot,x,y,tr1,tr2,used,choice2,field,turn):
    ...
    if x.item=="Custap Berry[Used]" and x.priority==True and used not in typemoves.prioritymove:    
        em.add_field(name=f"{await itemicon(x.item.replace('[Used]',''))} {x.nickname}'s {x.item}:",value=f"{x.item.replace('[Used]','')} will let {x.nickname} move first!")
    #Choice Item        
    ...
    #Checks Freeze                
        ...
        if used in typemoves.thawmove:       
            em.add_field(name="Status:",value=f"{y.nickname} thawed out.")
            y.status="Alive"       
    if x.status=="Frozen":
        ch=random.randint(1,10)
        if ch<=3 or used in typemoves.thawmove:
            em.add_field(name="Status:",value=f"{x.nickname} thawed out.")
            x.status="Alive"
        else:
    ...
    if (choice2 in typemoves.buffmove or choice2=="None") and used in typemoves.protectmoves:
        ...
        used="None"
    #Consecutive Protect     
    if x.protect=="Pending" and used in ["Protect","Spiky Shield","King's Shield","Baneful Bunker","Obstruct","Silk Trap"]:
        em.add_field(name=f"{x.nickname} used {used}!",value="It failed.")
        used="None"                 
        x.protect=False   
    ...
    if y.ability=="Disguise" and used not in typemoves.statusmove+typemoves.buffmove+typemoves.zmoves+typemoves.multimove+typemoves.abilityigmoves and x.ability not in ["Mold Breaker","Teravolt","Turboblaze","Propeller Tail","Stalwart"] and used!="None":
        em.add_field(name=f"{x.nickname} used {used}!",value=f"{y.icon} {y.nickname}'s {y.ability}!\n{y.icon} {y.nickname}'s disguise was busted!")
...
#        em.add_field(name="Consecutive use:",value=f"Cannot use {x.use} consecutively!")
    ...
    if used=="Assist" and canatk==True:
        ...
        em.add_field(name=f"{x.nickname} used Assist!",value=f"It will pick a random ally move! Assist turned into {used}!")          
    #Me First        
    ...
    if x.ability=="Prankster" and "Dark" in (y.primaryType,y.secondaryType,y.teraType) and used in typemoves.statusmove:
        used="None"
        em.add_field(name=f"{x.icon} {x.nickname}'s Prankster!",value=f"{y.nickname} is immune to {x.nickname}'s Prankster due to being partially Dark-Type!")                   
    #Good as Gold        
    if y.ability=="Good as Gold" and used in typemoves.statusmove and used not in ["Stealth Rock","Haze","Toxic Spikes","Protect","Spiky Shield","Baneful Bunker","King's Shield","Silk Trap"]+typemoves.healingmoves+typemoves.buffmove:
        em.add_field(name=f"{x.nickname} used Me First!",value=f"{y.icon} {y.nickname}'s {y.ability} prevented {used}!")
        used="None"              
    #Encore   
    ...
    if used in typemoves.powdermoves and y.item=="Safety Googles":
        em.add_field(name=f"{y.icon} {y.nickname}'s {await itemicon(y.item)} {y.item}!",value=f"{y.icon} {y.nickname}'s {y.item} protected it from {x.nickname}'s {used}!")
        used="None"  
    #Soundproof            
    if used in typemoves.soundmoves and y.ability=="Soundproof":
        em.add_field(name=f"{x.nickname} used {used}!",value=f"{y.icon} {y.nickname}'s Soundproof!")
        used="None"
    #Bulletproof        
    if used in typemoves.bulletmove and y.ability=="Bulletproof":
        em.add_field(name=f"{x.nickname} used {used}!",value=f"{y.icon} {y.nickname}'s Bulletproof!")
        used="None"
    #Fluffy        
    ...
    if x.ability=="Truant" and used!="Slack Off":
        if x.truant==True:
            em.add_field(name=f"{x.icon} {x.nickname}'s {x.ability}!",value=f"{x.nickname} is loafing around!")
            used="None"
            x.truant=False
        else:
            x.truant=True
    ...
    if x.item=="Assault Vest" and used in typemoves.statusmove:
        em.add_field(name=f"{x.icon} {x.nickname}'s {await itemicon(x.item)} {x.item}!",value=f"Cannot use status moves while holding an Assault Vest.")
        used="None"        
    ...
    if x.precharge==True and len(x.moves)>0 and "Geomancy" not in x.moves and x.status not in ["Sleep","Frozen"] and canatk==True:
        ...
        if len(l)!=0:
            used=l[0]           
    if used not in ["Protect","Spiky Shield","King's Shield","Baneful Bunker","Obstruct","Silk Trap","Max Guard"]:
            x.protect=False          
    if y.protect==True and (y.dmax is True and used not in typemoves.buffmove and used not in ["G-Max One Blow","G-Max Rapid Flow"] and used!="None"):
            ...
            used="None"
            if used in ["Protect","Spiky Shield","King's Shield","Baneful Bunker","Obstruct","Silk Trap"]:
                used="None"     
    #Protection Moves                
    if y.dmax is False and y.protect==True and used not in typemoves.buffmove and (x.ability not in ["Infiltrator","Unseen Fist"]  and used not in ["Shadow Force","Phantom Force","Hyperspace Fury","Hyper Drill","Hyperspace Hole"] and used not in typemoves.maxmovelist and used not in typemoves.zmoves) and used!="None" :
        ...
        elif "Silk Trap" in y.use and used in typemoves.contactmoves and x.item not in ["Punching Glove","Protective Pads"]:
            await speedchange(em,x,y,-1)
        elif "Obstruct" in y.use and used in typemoves.contactmoves and x.item not in ["Punching Glove","Protective Pads"]:
            await spatkchange(em,x,y,-1)
        elif "King's Shield" in y.use and used in typemoves.contactmoves and x.item not in ["Punching Glove","Protective Pads"]:
            await atkchange(em,x,y,-1)
        elif "Baneful Bunker" in y.use and used in typemoves.contactmoves and x.item not in ["Punching Glove","Protective Pads"]:
            if x.status=="Alive":
    ...
    if used in function_map:
        ...
        em.add_field(name=f"Move:",value=f"{x.nickname} used Whirlwind!")  
        if len(tr2.pokemons)>1 and y.ability!="Suction Cups" and y.dmax is False:
            em.add_field(name=f"Effect:",value=f"{y.nickname} blew away with the wind.") 
            ...
            while True:
                ...
                if y!=l:
                    break
            em.add_field(name="Whirlwind:",value=f"{y.nickname} was dragged out!")         
            await entryeff(ctx,y,x,tr2,tr1,field,turn)
    elif used=="Dragon Tail":
        await dragontail(ctx,x,y,tr1,em,field,turn)
        if len(tr2.pokemons)>1 and y.ability!="Suction Cups" and y.dmax is False and "Fairy" not in (y.primaryType ,y.secondaryType ,y.teraType):
            y.atkb=y.defb=y.spatkb=y.spdefb=y.speedb=0
            l=y
            while True:
                if tr2.sub=="None":
                ...
                if y!=l:
                    break
            em.add_field(name="Dragon Tail:",value=f"{y.nickname} was dragged out!")         
            await entryeff(ctx,y,x,tr2,tr1,field,turn)       
    elif used=="Roar":
        em.add_field(name=f"Move:",value=f"{x.nickname} used Roar!")  
        if len(tr2.pokemons)>1 and y.ability!="Suction Cups" and y.dmax is False:
            y.atkb=y.defb=y.spatkb=y.spdefb=y.speedb=0
            l=y
            while True:
                ...
                if y!=l:
                    break
            em.add_field(name="Roar:",value=f"{y.nickname} was dragged out!")         
            await entryeff(ctx,y,x,tr2,tr1,field,turn)                 
        ...
        if choice2 in typemoves.statusmove or choice2 =="None":
            em.add_field(name=f"{x.nickname} used Sucker Punch!",value="It failed.")
        else:
            await suckerpunch(ctx,x,y,tr1,em,field,turn) 
    elif used=="Thunderclap":
        if choice2 in typemoves.statusmove or choice2 =="None":
            em.add_field(name=f"{x.nickname} used Thunderclap!",value="It failed.")
        else:
            await thunderclap(ctx,x,y,tr1,em,field,turn)             
    elif used=="Parting Shot":
        await partingshot(ctx,x,y,tr1,em,field,turn) 
        ...
        await stealthrock(ctx,x,y,tr1,tr2,em,field,turn)
    elif used=="Stone Axe":
        await stoneaxe(ctx,x,y,tr1,tr2,em,field,turn)
    elif used=="G-Max Stonesurge":
        await gmaxstonesurge(ctx,x,y,tr1,tr2,em,field,turn)
    elif used=="Brick Break":
        await brickbreak(ctx,x,y,tr1,tr2,em,field,turn)
    elif used=="Psychic Fangs":
        await psychicfangs(ctx,x,y,tr1,tr2,em,field,turn)        
    elif used=="Roar Of Time":
        await roaroftime(ctx,x,y,tr1,em,field,turn)
        x.recharge=True
    elif used=="Raging Bull":
        await ragingbull(ctx,x,y,tr1,tr2,em,field,turn)
    elif used=="G-Max Steelsurge":
        await gmaxsteelsurge(ctx,x,y,tr1,tr2,em,field,turn)
    elif used=="Toxic Spikes":
        await toxicspikes(ctx,x,y,tr1,tr2,em,field,turn)
    elif used=="Ceasless Edge":
        await ceaslessedge(ctx,x,y,tr1,tr2,em,field,turn)     
    elif used=="Spikes":
        await spikes(ctx,x,y,tr1,tr2,em,field,turn)
    elif used=="Sticky Web":
        await stickyweb(ctx,x,y,tr1,tr2,em,field,turn)            
    elif used=="Chilly Reception":
        await chillyreception(ctx,x,y,tr1,em,field,turn) 
        if len(tr1.pokemons)>1:
            x=await switch(ctx,bot,x,y,tr1,tr2,field,turn) 
    elif used=="Shed Tail":
        await shedtail(ctx,x,y,tr1,em,field,turn) 
        if len(tr1.pokemons)>1 or x.hp<(x.maxhp/2):
            x=await switch(ctx,bot,x,y,tr1,tr2,field,turn)
    elif used=="Teleport":
        em.add_field(name=f"Move:",value=f"{x.nickname} used Teleport!")
        if len(tr1.pokemons)>1:
            x=await switch(ctx,bot,x,y,tr1,tr2,field,turn)            
    elif used=="U-turn":
    ...
    if y.hp==yhp and used=="None":
        ...
        if x.item=="Blunder Policy":
            em.add_field(name=f"{x.icon} {x.nickname}'s {await itemicon(x.item)} {x.item}!",value=f"Blunder Policy was used upon miss!")
            item+="[Used]"
            await speedchange(em,x,x,2)
    if yhp==y.maxhp and y.hp<=0:
        if y.item=="Focus Sash" and used not in typemoves.multimove:
            em.add_field(name=f"{y.icon} {y.nickname}'s {await itemicon(y.item)} {y.item}!",value=f"{y.nickname} hung on using Focus Sash!")
            y.hp=1
            y.item+="[Used]"
            ...
            em.add_field(name=f"{y.icon} {y.nickname}'s {y.ability}!",value=f"{y.nickname} hung on using {y.ability}!")
            y.hp=1
    if y.item=="Focus Band" and used not in typemoves.multimove and y.hp<=0:
        ch=random.randint(1,10)
        if ch==1:
            em.add_field(name=f"{y.icon} {y.nickname}'s {await itemicon(y.item)} {y.item}!",value=f"{y.nickname} hung on using Focus Band.!")
            y.hp=1
    if "Gulp Missile" in y.ability and y.hp!=yhp:
        if "-" in y.ability:
            y.sprite=y.sprite.split("-")[0]+".gif"
            em.add_field(name=f"{y.icon} {y.nickname}'s {y.ability}!",value=f"{y.nickname} launched something at the target!")
    ...
    if x.dbond is True and y.hp<=0:
        x.hp=0
        em.add_field(name="Effect:",value=f"{y.nickname} took away {x.nickname} with it!")
    if y.ability=="Illusion" and "Zoroark" not in y.name and y.hp!=yhp:
        if y.primaryType=="Dark":
            y.name="Zoroark"
            em.add_field(name=f"{y.name}'s Illusion!",value=f"{y.icon} {y.nickname}'s Illusion wore off!")
            if "shiny" not in y.sprite:
                y.sprite="http://play.pokemonshowdown.com/sprites/ani/zoroark.gif"
            elif "shiny" in y.sprite:
                y.sprite="http://play.pokemonshowdown.com/sprites/ani-shiny/zoroark.gif"
        else:
            y.name="Hisuian Zoroark"
            em.add_field(name=f"{y.name}'s Illusion!",value=f"{y.icon} {y.nickname}'s Illusion wore off!")
            if "shiny" not in y.sprite:
                y.sprite="http://play.pokemonshowdown.com/sprites/ani/zoroark-hisui.gif"        
    elif y.ability in ["Gooey","Tangling Hair"] and me.ability not in ["Clear Body","Good as Gold"] and me.ability!="Long Reach" and used in typemoves.contactmoves and x.item not in ["Punching Glove","Protective Pads"] and y.hp!=yhp:
        ...
        x.infatuated=True
        em.add_field(name=f"{y.name}'s {y.ability}!",value=f"{x.nickname} is infatuated!")
        ...
        if ll==1 and x.status=="Alive":
            await poison(em,y,x,30)
        elif ll==2 and x.status=="Alive":
            await sleep(em,y,x,30)
        elif ll==3 and x.status=="Alive": 
            await paralyze(em,y,x,30)
    ...
    if ((x.ability in ["Poison Touch","Toxic Fangs","Toxic Drain"] and used in typemoves.contactmoves and x.item not in ["Punching Glove","Protective Pads"]) or x.ability=="Toxic Chain") and me==x:
    ...
    if used in typemoves.contactmoves and y.item=="Air Balloon" and x.item not in ["Punching Glove","Protective Pads"] and used not in typemoves.groundmoves and y.hp!=yhp:
        em.add_field(name=f"{y.icon} {y.nickname}'s {await itemicon(y.item)} {y.item}!",value=f"{y.icon} {y.nickname}'s Air Balloon popped off!")
        y.item+="[Used]"   
    if yhp!=y.hp and y==they:
        ...
        if y.item=="Rocky Helmet" and x.ability!="Magic Guard" and used in typemoves.contactmoves and x.item not in ["Punching Glove","Protective Pads"]:
            ...
            em.add_field(name=f"{y.icon} {y.nickname}'s {await itemicon(y.item)} {y.item}!",value=f"{x.nickname} was hurt by {y.icon} {y.nickname}'s Rocky Helmet!")
            if me.hp<0:
                me.hp=0      
            ...
            while y!=they:
                y=random.choice(tr2.pokemons)
                tr2.party=await partyup(tr2,y)
            entr1yeff(y,x,tr2,tr1,field,turn)  
...
#LIFE ORB                    
        if me.item=="Life Orb" and me.ability!="Magic Guard":
            ...
            em.add_field(name=f"{await itemicon(me.item)} {me.nickname}'s {me.item}:",value=f"{me.nickname} lost some of its HP")
            if x.hp<0:
                x.hp=0                
    #Persim    
    if y.item=="Persim Berry" and y.confused==True and y.hp>0 and x.ability not in ["Unnerve","As One"]:
        if y.ability=="Cheek Pouch" and y.hp<y.maxhp:
            y.hp+=(y.maxhp/3)
        em.add_field(name=f"{y.icon} {y.nickname}'s {await itemicon(y.item)} {y.item}!",value=f"{y.item} cured {y.icon} {y.nickname}'s confusion!")
        y.confused=False
        y.item+="[Used]"     
   #Persim   
    if x.item=="Persim Berry" and x.confused==True and x.hp>0 and y.ability not in ["Unnerve","As One"]:
        if x.ability=="Cheek Pouch" and x.hp<x.maxhp:
            x.hp+=(x.maxhp*0.33)
        em.add_field(name=f"{x.icon} {x.nickname}'s {await itemicon(x.item)} {x.item}!",value=f"{x.item} cured {x.nickname}'s confusion!")
        x.confused=False
        x.item+="[Used]"        
    #Cheri        
    if x.item=="Cheri Berry" and x.status=="Paralyzed" and x.hp>0 and y.ability not in ["Unnerve","As One"]:
        if x.ability=="Cheek Pouch" and x.hp<x.maxhp:
            x.hp+=(x.maxhp*0.33)
        em.add_field(name=f"{x.icon} {x.nickname}'s {await itemicon(x.item)} {x.item}!",value=f"{x.item} cured {x.nickname}'s paralysis!")
        x.status="Alive"
        x.item+="[Used]"
    #Cheri        
    if y.item=="Cheri Berry" and y.status=="Paralyzed" and y.hp>0 and y.ability not in ["Unnerve","As One"]:
        if y.ability=="Cheek Pouch" and y.hp<y.maxhp:
            y.hp+=(y.maxhp*0.33)
        em.add_field(name=f"{y.icon} {y.nickname}'s {await itemicon(y.item)} {y.item}!",value=f"{y.item} cured {y.icon} {y.nickname}'s paralysis!")
        y.status="Alive"
        y.item+="[Used]"     
    #Rawst
    if x.item=="Rawst Berry" and x.status=="Burned" and x.hp>0 and x.ability not in ["Unnerve","As One"]:
        if x.ability=="Cheek Pouch" and x.hp<x.maxhp:
            x.hp+=(x.maxhp*0.33)
        em.add_field(name=f"{x.icon} {x.nickname}'s {await itemicon(x.item)} {x.item}!",value=f"{x.item} cured {x.nickname}'s burn!")
        x.status="Alive"
        x.item+="[Used]"
    #Rawst
    if y.item=="Rawst Berry" and y.status=="Burned" and y.hp>0 and y.ability not in ["Unnerve","As One"]:
        if y.ability=="Cheek Pouch" and y.hp<y.maxhp:
            y.hp+=(y.maxhp*0.33)
        em.add_field(name=f"{y.icon} {y.nickname}'s {await itemicon(y.item)} {y.item}!",value=f"{y.item} cured {y.icon} {y.nickname}'s burn!")
        y.status="Alive"
        y.item+="[Used]"        
    #Chesto 
    if x.item=="Chesto Berry" and x.status=="Sleep" and x.hp>0 and y.ability not in ["Unnerve","As One"]:
        if x.ability=="Cheek Pouch" and x.hp<x.maxhp:
            x.hp+=(x.maxhp*0.33)
        em.add_field(name=f"{x.icon} {x.nickname}'s {await itemicon(x.item)} {x.item}!",value=f"{x.item} cured {x.nickname}'s sleep!")
        x.status="Alive"
        x.item+="[Used]"
   #Chesto
    if y.item=="Chesto Berry" and y.status=="Sleep" and y.hp>0 and x.ability not in ["Unnerve","As One"]:
        if y.ability=="Cheek Pouch" and y.hp<y.maxhp:
            y.hp+=(y.maxhp/3)
        em.add_field(name=f"{y.icon} {y.nickname}'s {await itemicon(y.item)} {y.item}!",value=f"{y.item} cured {y.icon} {y.nickname}'s sleep!")
        y.status="Alive"
        y.item+="[Used]"
   #Aspear
    if x.item=="Aspear Berry" and x.status=="Frozen" and x.hp>0 and y.ability not in ["Unnerve","As One"]:
        if x.ability=="Cheek Pouch" and x.hp<x.maxhp:
            x.hp+=(x.maxhp*0.33)
        em.add_field(name=f"{x.icon} {x.nickname}'s {await itemicon(x.item)} {x.item}!",value=f"{x.item} cured {x.nickname}'s freeze!")
        x.status="Alive"
        x.item+="[Used]"
    #Aspear
    if y.item=="Aspear Berry" and y.status=="Frozen" and y.hp>0 and x.ability not in ["Unnerve","As One"]:
        if y.ability=="Cheek Pouch" and y.hp<y.maxhp:
            y.hp+=(y.maxhp/3)
        em.add_field(name=f"{y.icon} {y.nickname}'s {await itemicon(y.item)} {y.item}!",value=f"{y.item} cured {y.icon} {y.nickname}'s freeze!")
        y.status="Alive"
        y.item+="[Used]"
    #Pecha
    if x.item=="Pecha Berry" and x.status=="Badly Poisoned" and x.hp>0 and y.ability not in ["Unnerve","As One"]:
        if x.ability=="Cheek Pouch" and x.hp<x.maxhp:
            x.hp+=(x.maxhp*0.33)
        em.add_field(name=f"{x.icon} {x.nickname}'s {await itemicon(x.item)} {x.item}!",value=f"{x.item} cured {x.nickname}'s poison!")
        x.status="Alive"
        x.item+="[Used]"
    #Pecha
    if y.item=="Pecha Berry" and y.status=="Badly Poisoned" and y.hp>0 and x.ability not in ["Unnerve","As One"]:
        if y.ability=="Cheek Pouch" and y.hp<y.maxhp:
            y.hp+=(y.maxhp/3)
        em.add_field(name=f"{y.icon} {y.nickname}'s {await itemicon(y.item)} {y.item}!",value=f"{y.item} cured {y.icon} {y.nickname}'s poison!")
        y.status="Alive"
        y.item+="[Used]"
    #Lum
    if x.item=="Lum Berry" and x.status!="Alive"and x.hp>0 and y.ability not in ["Unnerve","As One"]:
        if x.ability=="Cheek Pouch" and x.hp<x.maxhp:
            x.hp+=(x.maxhp*0.33)
        em.add_field(name=f"{x.icon} {x.nickname}'s {await itemicon(x.item)} {x.item}!",value=f"{x.item} cured {x.nickname}'s status condition!")
        x.status="Alive"
        x.item+="[Used]"        
    #Lum
    if y.item=="Lum Berry" and y.status!="Alive" and y.hp>0 and x.ability not in ["Unnerve","As One"]:
        if y.ability=="Cheek Pouch" and y.hp<y.maxhp:
            y.hp+=(y.maxhp/3)
        em.add_field(name=f"{y.icon} {y.nickname}'s {await itemicon(y.item)} {y.item}!",value=f"{y.item} cured {y.icon} {y.nickname}'s status condition!")
        y.status="Alive"
        y.item+="[Used]"
    #Sitrus
    if y.hp<=y.maxhp/2 and y.hp>0:
        if y.item=="Sitrus Berry" and x.ability not in ["Unnerve","As One"]:
            ...
            em.add_field(name=f"{y.icon} {y.nickname}'s {await itemicon(y.item)} {y.item}!",value=f"{y.nickname} restored Hp using its {y.item}!")
            y.item+="[Used]"                
    #Sitrus
    if x.hp<=x.maxhp/2 and x.hp>0:
        if x.item=="Sitrus Berry" and y.ability not in ["Unnerve","As One"]:    
            ...
            em.add_field(name=f"{x.icon} {x.nickname}'s {await itemicon(x.item)} {x.item}!",value=f"{x.nickname} restored Hp using its {x.item}!")
            x.item+="[Used]"    
    if x.hp>0:
        if x.item=="Shell Bell" and y.dmgtaken>0 and y.protect==False and x.recharge==False:
            x.hp+=(y.dmgtaken/8)
            em.add_field(name=f"{x.icon} {x.nickname}'s {await itemicon(x.item)} {x.item}!",value=f"{x.nickname} restored a little HP using its Shell Bell!")            
    if y.hp>0:
        await berry(em,y,x,xhp,yhp,turn)     
        if y.hp<=(y.maxhp/2)  and yhp>(y.maxhp/2):
            if y.ability=="Berserk":
                em.add_field(name=f"{y.icon} {y.nickname}'s {y.ability}!",value=f"{y.nickname} went berserk!")
                await spatkchange(em,y,y,1)
            if y.ability=="Anger Shell":
                em.add_field(name=f"{y.icon} {y.nickname}'s {y.ability}!",value=f"{y.nickname} became extremely angry!")
    ...
    if y.ability=="Innards Out" and y==they and y.hp<=0:
        em.add_field(name=f"{y.icon} {y.nickname}'s {y.ability}!",value=f"{y.nickname} launched a huge punch!")
        x.hp-=yhp    
    if x.item=="Throat Spray" and used in typemoves.soundmoves:
        await spatkchange(em,x,x,1)
        em.add_field(name=f"{x.icon} {x.nickname}'s {await itemicon(x.item)} {x.item}!",value=f"The Throat Spray raised {x.nickname}'s Special Attack!")
        x.item+="[Used]"           
