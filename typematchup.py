...
async def weakness(ctx,x,y,field,em):
    ...
    ty=1
    stab=1
    if x.use in typemoves.contactmoves and x.ability=="Tough Claws" and "Neutralizing Gas" not in y.ability:
        em.add_field(name=f"{x.name}'s {x.ability}!",value="Contact move was boosted!")
        eff*=1.33
    if x.use in typemoves.soundmoves and x.ability=="Punk Rock" and "Neutralizing Gas" not in y.ability:
        em.add_field(name=f"{x.name}'s {x.ability}!",value="Sound move was boosted!")
        eff*=1.3
    if x.use in typemoves.soundmoves and y.ability=="Punk Rock" and "Neutralizing Gas" not in x.ability:
        em.add_field(name=f"{y.name}'s {y.ability}!",value="Sound move was reduced!")
        eff*=0.5
    if x.charged==True:
    ...
    if "Absolute Zero" in (x.ability,y.ability) and x.atktype=="Water" and "Neutralizing Gas" not in (y.ability,x.ability):
        if x.ability=="Absolute Zero":
            em.add_field(name=f"{x.name}'s {x.ability}!",value="Absolute Zero turned the Water-type move into Ice-type move!")
        if y.ability=="Absolute Zero":
            em.add_field(name=f"{y.name}'s {y.ability}!",value="Absolute Zero turned the Water-type move into Ice-type move!")
        x.atktype="Ice"
    #UNAWARE(y EFFECT)
    ...
    if x.ability in ["Protean","Libero"] and x.primaryType!=x.atktype and "Neutralizing Gas" not in y.ability:
        ...
        em.add_field(name=f"{x.name}'s {x.ability}!",value=f"{x.name} changed it's type to {x.primaryType}!")
    #Color Change
    if x.ability in ["Color Change"] and x.primaryType!=y.atktype and "Neutralizing Gas" not in y.ability:
    ...
    if (y.hp == y.maxhp and y.ability in ["Multiscale", "Blubber Defense", "Shadow Shield"] and x.ability not in ["Mold Breaker","Teravolt","Turboblaze","Propeller Tail","Stalwart","Neutralizing Gas"] and x.use not in typemoves.abilityigmoves):
        em.add_field(name=f"{y.name}'s {y.ability}!",value=f"Damage was reduced in full HP!")
        eff *= 0.5
    #SOLAR POWER
    if x.ability=="Solar Power" and field.weather in ["Sunny","Desolate Land"] and "Cloud Nine" not in (x.ability,y.ability) and "Neutralizing Gas" not in y.ability:
        em.add_field(name=f"{x.name}'s {x.ability}!",value=f"Damage boosted in sunlight!")
        eff*=1.5
    #ANALYTIC
    if x.ability=="Analytic" and "Neutralizing Gas" not in y.ability:
        if x.speed<y.speed:
            em.add_field(name=f"{x.name}'s {x.ability}!",value=f"Damage boosted for moving slow!")
            eff*=1.3
    #NORMALIZE 
    if x.ability=="Normalize" and "Neutralizing Gas" not in y.ability:
        x.atktype="Normal"
    #CHANGING NORMAL TYPE MOVES
    if x.atktype=="Normal":
        if x.ability in ["Pixilate","Aerilate","Galvanize","Refrigerate","Liquid Voice"] and "Neutralizing Gas" not in y.ability:
    ...
    if x.atktype=="Ground":
        ...
        if y.teraType in groundwk:
            ty/=2
        if (y.primaryType in groundimmune or y.secondaryType in groundimmune and y.teraType=="???") or y.teraType in groundimmune and y.item!="Ring Target" and x.grav==False:
            if y.item!="Iron Ball":
                ty*=0
    #GRASS
    if x.atktype=="Grass":
    ...
    if x.atktype=="Fire":
        ...
        if y.ability=="Dry Skin" and "Cloud Nine" not in (x.ability,y.ability):
            print(f" {y.name}'s {y.ability}!")
            eff*=1.25
        if y.ability in ["Thick Fat","Heatproof"] and x.ability not in ["Mold Breaker","Teravolt","Turboblaze","Propeller Tail","Stalwart","Neutralizing Gas"] and x.use not in typemoves.abilityigmoves:
            print(f" 🌡️ {y.name}'s {y.ability}!")
            eff*=0.5
...
async def isCrit(em,tr1,x,y,num="None"):
    ...
    if x.item in ["Scope Lens","Razor Claw"]:
        num*=2
    if x.ability in ["Super Luck","Winged Fury"]:
        num*=2
    ...
    if x.ability=="Merciless" and y.status in ["Badly Poisoned","Poisoned"]:
        crit=1
    if crit==1 and y.ability not in ["Shell Armor","Battle Armor"]:
        ...
        if x.ability=="Sniper":
            ...
            return 2
    if y.ability in ["Shell Armor","Battle Armor"] and crit==1:
        return 1
    else:
        return 1                        
