...
async def profile(ctx):
    ...
    try:
        ...
        for i in team:
            ll+=1
            data.add_field(name=f"#{sq[ll-1]} {i.icon} {i.name} {await teraicon(i.tera)}",value=f"**Ability:** {i.ability}\n**Item:** {await itemicon(i.item)} {i.item}")
    except:
...
async def multiplayer(ctx, p1, p2):
    ...
    or y.item == "Lagging Tail"
                ...
                if xPriority and x.item=="Quick Claw":
    ...
    or x.priority
    or (x.ability == "Prankster" and choice2 in typemoves.statusmove and "Dark" not in (x.primaryType, x.secondaryType))
    ...
    or x.item == "Lagging Tail"
                ...
                if yPriority and y.item=="Quick Claw":
