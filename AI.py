...
async def moveAI(x,y,tr1,tr2,field):
    ...
    if y.ability in ["Volt Absorb","Motor Drive","Lightning Rod"]:
        myimmunemove+=list(set(mymove). intersection (typemoves.electricmoves))     
    if y.ability in ["Flash Fire","Well-baked Body"] or field.weather=="Primordial Sea":
        myimmunemove+=list(set(mymove). intersection (typemoves.firemoves))    
    if y.ability in ["Levitate","Earth Eater"] or y.item=="Air Balloon":
        myimmunemove+=list(set(mymove). intersection (typemoves.groundmoves))               
        
    if "Water" in weaklist and y.ability not in ["Water Absorb","Water Compaction","Storm Drain","Dry Skin"]:
        emove+=list(set(mymove). intersection(typemoves.watermoves))
    if "Fire" in weaklist and y.ability not in ["Well-baked Body","Flash Fire"]:
        emove+=list(set(mymove). intersection(typemoves.firemoves))
    if "Grass" in weaklist and y.ability not in ["Sap Sipper"]:
        emove+=list(set(mymove). intersection(typemoves.grassmoves))          
    if "Rock" in weaklist and y.ability not in ["Mountaineer"]:
        emove+=list(set(mymove). intersection(typemoves.rockmoves))     
    if "Ground" in weaklist and y.ability not in ["Levitate","Earth Eater"] and y.item!="Air Balloon":
        emove+=list(set(mymove). intersection(typemoves.groundmoves)) 
    if "Electric" in weaklist and y.ability not in ["Volt Absorb","Lightning Rod","Motor Drive"]:
